let drawImageFromUrl = function() { alert("Uninit"); };
...
document.getElementsByTagName("body")[0].innerHTML += `
    <input placeholder='Url to image' id='imgUrl' class='hackinput'></input>
    <button onclick='drawImageFromUrl();' class='hackbtn' id='drawBtn'>Draw</button>
    <input placeholder='Quality 1 - 10 (smaller better)' id='quality' class='hackinput' style='margin-top: 33px'></input>
    <input placeholder='Scale 1 - 10' id='scale' class='hackinput' style='margin-top: 66px'></input>
`;
...
! function(t, e, n, o, r) {
    ...
    function T(t, e, n) {
        ...
        // Drawing function
        drawImageFromUrl = function() {
            var img = new Image();
            img.setAttribute('crossOrigin', 'anonymous');

            // available colors sorted by index
            let colors = [
                [255, 255, 255],
                [0, 0, 0],
                [192, 192, 192],
            ...
            img.onload = function() {
                ...
                let s = document.getElementById("scale").value; // scale
                let f = document.getElementById("quality").value; // img quality
                ...
                let height = parseInt(canvas.height / f);
                let width = parseInt(canvas.width / f);

                let image = [];

                // create virtual image with available colors
                for (let x = 0; x < width; ++ x) {
                    for (let y = 0; y < height; ++ y) {

                        let pixelData = ctx.getImageData(x * f, y * f, 1, 1).data; // rgba
                        let brushColor = 0;
                        let oldDiff = 255;

                        for (let c = 0; c < colors.length; ++ c) {
                            let cDiff = Math.sqrt(
                                Math.pow(colors[c][0]-pixelData[0], 2) +
                                Math.pow(colors[c][1]-pixelData[1], 2) +
                                Math.pow(colors[c][2]-pixelData[2], 2)
                            );
                        ...
                        image[y + x * height] = pixelData[3] == 0 ? -1 : brushColor;
                    }
                }
                ...
                for (let x = 0; x < width; ++ x) {
                    for (let y = 0; y < height; ++ y) {
                        let endY = y;
                        let brushColor = image[y + x * height];

                        // search last eq color
                        for (; endY < height && image[endY + x * height] == brushColor; ++ endY);

                        // draw line
                        if (brushColor != -1) {
                            I(ut.createDrawCommandLine(brushColor, ut.brush.thickness, x * s, y * s, x * s, endY * s));
                        }
            ...
            img.src = "https://cors-anywhere.herokuapp.com/" + document.getElementById("imgUrl").value;
        }
    }
